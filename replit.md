...
Integration with Cisco ASR/ISR e Mikrotik supports PPPoE concentrator functions, including interface discovery, PPPoE username retrieval, and MAC address limitations. It collects ONU ID via OLT and supports vendor auto-detection.

**Concentrator interface discovery — safety rules** (detalhes nos comentários dos respectivos arquivos):
- `lookupVlanInterfaceIndex` (`server/concentrator.ts`): match em 3 passos seguros — (1) exato case-sensitive, (2) exato normalizado, (3) substring forward exigindo candidato ÚNICO. Substring reversa removida (causava match em interface mãe agregadora quando o nome cadastrado a continha como prefixo).
- IP route lookup (`server/monitoring.ts` ~1888 primário, ~1976 backup): além de `isPlausibleSubscriberInterface` (formato `Vi*`/`pppoe-*`/`ppp\d`/`<pppoe-`), valida o alias descoberto contra a identidade conhecida (`pppoeUser` OU `snmpInterfaceAlias`) via `aliasMatchesKnownIdentity` (normaliza lowercase, strip prefixos `pppoe-`/`pppoe_`/`<pppoe-`, strip `@realm`). Sem identidade conhecida, cai no comportamento permissivo antigo.

**PPPoE session contamination — proteções** (detalhes nos comentários do código):
- **Cisco Vi destruída (`verifyInterfaceAtIndex`)**: distingue timeout (mantém) vs `noSuchInstance` em ifName+ifDescr+ifAlias (`interfaceMissing=true`). Pra Cisco Vi com concentrador, limpa imediatamente o ifIndex sem esperar `IFINDEX_MISMATCH_THRESHOLD`.
- **Cisco Vi anti-contaminação de pppoeUser**: auto-store de `snmpInterfaceAlias`/`pppoeUser` é BLOQUEADO em Cisco Vi com concentrador (slot Vi3.X é alocado pra cliente arbitrário a cada sessão; auto-aprender grava o invasor). `pppoeUser` SÓ pode vir do cadastro Voalle/manual. Para BDI/sub-interfaces estáticas/GPON VLANs (ifIndex fixos por cliente), auto-store continua valendo.
- **Cisco Vi user mismatch**: valida `pppoeUser` cadastrado contra `ifAlias` E `ifDescr` (este só se NÃO bater em `^Virtual-Access`/`^ViX.Y$`, padrões técnicos). Match em qualquer um → OK; nenhum bate → limpa ifIndex.
- **Cisco Vi RADIUS↔SNMP cross-check** (`RADIUS_IP_VERIFY_INTERVAL_MS=5min`, `lastRadiusIpVerification`): após match alias/descr passar, busca `framedIpAddress` no RADIUS via `getRadiusSessionByUsername` e compara `lookupIfIndexByIp(framedIp)` com o ifIndex atual. Se diferente, ATUALIZA direto (não limpa), zera `ifIndexMismatchCount`, limpa `previousTrafficData`. Cooldown marcado SÓ se RADIUS responder.
- **Non-Cisco (Mikrotik etc) user mismatch** (`server/monitoring.ts` ~2568, paralelo ao Cisco Vi): roda quando link é concentrator-link com `pppoeUser`, `authType !== 'corporate'`, `!vlanInterface`, SNMP retornou alias/name E `isPlausibleSubscriberInterface` confirma formato PPPoE. Compara SÓ contra `pppoeUser` do cadastro (`snmpInterfaceAlias` salvo pode estar contaminado). Se não bate → limpa `snmpInterfaceIndex/Name/Descr/Alias` + `ifIndexMismatchCount` alto. Auto-corrige casos legados (ex.: JE.eleonora.cruz após reboot OLT) sem intervenção manual.
