...
**PPPoE user mismatch detector pra Mikrotik/não-Cisco (auto-correção retroativa)**: detector análogo ao Cisco Vi mismatch, em `server/monitoring.ts` ~2559. Roda no mesmo bloco de verificação de interface (`IFNAME_VERIFY_INTERVAL_MS` cooldown padrão), condicional a: link é concentrator-link, tem `pppoeUser` cadastrado, NÃO é Cisco Vi, e SNMP retornou `ifAlias` ou `ifName`. Compara o alias atual SNMP contra **somente** o `pppoeUser` do cadastro (NÃO usa `snmpInterfaceAlias` salvo, que pode estar contaminado de descobertas anteriores) via `aliasMatchesKnownIdentity`. Se não bate → loga "PPPoE USER MISMATCH (non-Cisco)" e limpa `snmpInterfaceIndex/Name/Descr/Alias` + seta `ifIndexMismatchCount` alto, forçando re-descoberta no próximo ciclo (agora protegida pelo anti-cross-contamination do IP route lookup). Pega CASOS LEGADOS: links já contaminados em produção (como o JE.eleonora.cruz após o reboot da OLT) auto-corrigem no próximo ciclo de verificação sem intervenção manual, contanto que o cadastro `pppoeUser` esteja correto.
