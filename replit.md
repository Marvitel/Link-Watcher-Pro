...
**Cross-interface delta protection** (`server/monitoring.ts`): SNMP octet counters are per-interface. When `handleIfIndexAutoDiscovery` returns a new `ifIndex`, the cached `previousTrafficData` is from the OLD interface — computing `delta = new_ifIndex_counter - old_ifIndex_counter` produces physically impossible spikes (e.g., 35 Tbps). The post-discovery branch in `runOnce` now `delete()`s the cache before fetching with the new ifIndex and stores the sample as a fresh baseline (no delta this cycle; the next cycle calculates correctly).

**Per-link sanity clamp** (shared helpers `getClampCeilingMbps()` / `clampBandwidthSample()` in `server/monitoring.ts`): discards samples that physically can't exist on that link. Ceiling = `max(linkBandwidth × LINK_BANDWIDTH_CLAMP_MULTIPLIER (=5), MIN_PER_LINK_CLAMP_MBPS (=200))`. Examples: a 50 Mbps client is clamped at 250 Mbps, a 1 Gbps client at 5 Gbps, a 20 Gbps BGP uplink at 100 Gbps — preserves legitimate burst/DDoS peaks (which saturate at the link's physical ceiling) while catching cross-interface delta outliers (which always run >100× the link's capacity). A global fallback `MAX_REASONABLE_MBPS = 30_000` (30 Gbps = 2× Marvitel's largest link) applies when bandwidth is unknown.

**Three clamp call sites** (all must pass `link.bandwidth`):
1. Main flow `calculateBandwidth(...)` in `runOnce` (line ~2167) — clamps the metric inserted into `metrics` table.
2. Additional interfaces `calculateBandwidth(...)` in `runOnce` (line ~3544) — clamps each row inserted into `traffic_interface_metrics`.
3. `mainGraphMode` aggregate/single block (line ~3636) — calls `clampBandwidthSample()` on the aggregated/copied `mainDownload`/`mainUpload` BEFORE updating `links.currentDownload`/`currentUpload` and the metric row UPSERT. Without this, summing several already-clamped interfaces in `aggregate` mode (or copying a single interface that points to an uplink/switch larger than the parent link in `single` mode) overwrote the clamped main-flow row with values like 3 Gbps in a 100 Mbps link — exactly what was observed in PETROX_13/PETROX_SKINA before the fix (24+15 sustained samples surviving as raw rows because the per-interface clamp succeeded but the aggregate-then-overwrite step bypassed it).
