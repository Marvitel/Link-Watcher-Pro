...
**Cisco Vi RADIUS↔SNMP cross-check (auto-correção)**: Camada extra com cooldown próprio (`RADIUS_IP_VERIFY_INTERVAL_MS = 5min`, mapa `lastRadiusIpVerification`), executada dentro do mesmo bloco do verify Cisco Vi, somente após o match alias/descr passar. Fluxo: (1) busca sessão RADIUS via `getRadiusSessionByUsername(link.pppoeUser)` → pega `framedIpAddress`; (2) chama `lookupIfIndexByIp(concentrator, framedIp, snmpProfile)` no Cisco do `link.concentratorId` (usa `ipCidrRouteIfIndex`/`ipRouteIfIndex`); (3) compara o `ifIndex` retornado com `trafficSourceIfIndex`. Se diferente, ATUALIZA direto `snmpInterfaceIndex`/`snmpInterfaceName` pro novo ifIndex (não limpa pra re-descoberta), zera o `ifIndexMismatchCount`, limpa `previousTrafficData` (counters de outra interface não podem ser comparados) e ressincroniza `lastIfNameVerification`. Vantagem: prova definitiva de que estávamos no Vi errado, independente do que `ifAlias`/`ifDescr` digam — cobre o caso em que o operador ainda não configurou `description=username` ou os 2 clientes coincidentemente tinham aliases parecidos. Não roda quando o link não tem `pppoeUser` cadastrado.
...
**Corporate VLAN lookup anti-parent-interface match (`lookupVlanInterfaceIndex` em `server/concentrator.ts` ~1900)**: A busca por `links.vlanInterface` no concentrador usava 3 condições de match no loop de `ifName`/`ifDescr`: igualdade normalizada, `normalizedName.includes(normalizedSearch)` E `normalizedSearch.includes(normalizedName)`. A terceira (substring REVERSA) era o bug: aceitava qualquer interface cujo nome fosse SUBSTRING do nome cadastrado. Caso real (link 497 ASSEC-CEHOP, DATACOM OLT): cadastro `vlan1sfp28-11-OLT-DATACOM-TG-1/1/1-gpon.3-vlan.721-ASSEC-CEHOP` continha como prefixo o nome da interface uplink agregadora `sfp28-11-OLT-DATACOM-TG-1/1/1` (ifIndex 20) — o sistema saltava da sub-interface VLAN correta (ifIndex 247) pra interface MÃE, coletando tráfego agregado de todos os clientes do PON. Fix: helper interno `findMatch(data, label)` faz 3 passos seguros — (1) match exato case-sensitive, (2) match exato normalizado (lowercase, sem `.-_` espaço), (3) `normalizedName.includes(normalizedSearch)` **direção segura, exigindo candidato ÚNICO** (se >1 match, loga "AMBÍGUO" e retorna null pra não escolher errado). Substring reversa eliminada de vez. Aplicado a `ifName` E `ifDescr` (helper compartilhado).

**IP route lookup anti-cross-contamination (Mikrotik & Cisco)**: o fallback de descoberta de ifIndex via IP route lookup (`server/monitoring.ts`, callsites primário ~1888 e backup ~1976) agora valida o alias da interface descoberta contra a identidade conhecida do link (`pppoeUser` ou `snmpInterfaceAlias` previamente gravado) via helper `aliasMatchesKnownIdentity`. Antes, só verificava se o **formato** parecia sessão PPPoE (`isPlausibleSubscriberInterface` aceitava qualquer `pppoe-*`), o que permitia gravar o ifIndex de OUTRO cliente quando o IP do link estava momentaneamente roteado pra sessão errada (reboot do BNG/OLT, ARP cache antigo, pool reciclando IPs). Caso real: link `JE.eleonora.cruz` com alias `pppoe-JE.eleonora.cruz` teve OLT reiniciada; durante o reboot, IP route lookup no Mikrotik retornou `pppoe-2780rildongregorio` (outro cliente) — passou no `looksLikeSession` (prefixo `pppoe-`) e o sistema gravou o ifIndex errado. O helper normaliza (lowercase, strip de prefixos `pppoe-`/`pppoe_`/`<pppoe-`, strip `@realm`) e aceita match contra QUALQUER identidade conhecida (`pppoeUser` OU `snmpInterfaceAlias`), pra não travar descobertas legítimas quando cliente troca username. Quando link sem identidade conhecida, mantém o comportamento permissivo antigo (`looksLikeSession`). Aplicado nos 2 callsites (primário + backup).

**PPPoE user mismatch detector pra Mikrotik/não-Cisco (auto-correção retroativa)**: detector análogo ao Cisco Vi mismatch, em `server/monitoring.ts` ~2568. Roda no mesmo bloco de verificação de interface (`IFNAME_VERIFY_INTERVAL_MS` cooldown padrão), condicional a (4 guards): link é concentrator-link, tem `pppoeUser` cadastrado, NÃO é Cisco Vi, `authType !== 'corporate'` E `!vlanInterface` (corporate VLAN tem detector próprio em `lookupVlanInterfaceIndex`; aqui pppoeUser pode ser residual de migração), SNMP retornou `ifAlias` ou `ifName`, E `isPlausibleSubscriberInterface(actualName, actualAlias, pppoeUser)` confirma que SNMP parece uma sessão PPPoE (regex `Vi*`, `pppoe-*`, `ppp\d`, `<pppoe-`, etc, ou contém o username). Compara o alias atual SNMP contra **somente** o `pppoeUser` do cadastro (NÃO usa `snmpInterfaceAlias` salvo, que pode estar contaminado de descobertas anteriores) via `aliasMatchesKnownIdentity`. Se não bate → loga "PPPoE USER MISMATCH (non-Cisco)" e limpa `snmpInterfaceIndex/Name/Descr/Alias` + seta `ifIndexMismatchCount` alto, forçando re-descoberta no próximo ciclo (agora protegida pelo anti-cross-contamination do IP route lookup). Pega CASOS LEGADOS: links já contaminados em produção (como o JE.eleonora.cruz após o reboot da OLT) auto-corrigem no próximo ciclo de verificação sem intervenção manual, contanto que o cadastro `pppoeUser` esteja correto. Os guards evitam false-trigger em links corporate ou em links onde SNMP retornou interface não-PPPoE (reclassificação de link).
...
### PPPoE Session Detection (RADIUS + Mikrotik/Cisco fallback)
A UI do link mostra um badge/card de sessão PPPoE ativa via `GET /api/links/:id/pppoe-session`. O backend tenta primeiro `radacct` (FreeRADIUS via `getRadiusSessionByUsername`); se não encontrar, faz fallback no concentrador, **escolhendo o caminho pelo `concentrator.vendor`**:
- **Mikrotik** (`mikrotik`/`mikrotikls`/`routerboard`): API binária RouterOS em `/ppp/active` filtrada por `name=username` via `getMikrotikPppoeSessionByUsername`. Retorna `address`, `caller-id`, `uptime`.
- **Cisco** (`cisco`/`ciscosystems`/`ciscosystemsinc`): SNMP via `getCiscoPppoeSessionByUsername(concentrator, username, { ifIndex, monitoredIp, snmpProfile })`. Tenta primeiro o `snmpInterfaceIndex` salvo do link — SNMP get em ifName/ifDescr/ifAlias/ifOperStatus/ifLastChange + sysUpTime; se a Vi existe (não veio noSuchInstance), está `up(1)` E o alias OU descr bate com username (case-insensitive, ignora `@realm`, rejeita strings técnicas `^Virtual-Access` e `^ViX.Y$`), considera sessão ativa e calcula `uptimeSec` por `(sysUpTime - ifLastChange)/100`. Se não bater, faz fallback via `lookupIfIndexByIp(concentrator, monitoredIp)` na routing table do Cisco e revalida alias. `framedIpAddress` retornado = `link.monitoredIp` (Cisco SNMP simples não expõe o IP do PPPoE diretamente; o cross-check de IP confirma a sessão é desse IP).
- Vendors desconhecidos caem no caminho Mikrotik por compatibilidade legada.

O detector de rompimento massivo (`server/massive-outage-detector.ts`) usa o mesmo fallback em modo bulk **para Mikrotik**: agrupa links offline por `concentratorId` e chama `getMikrotikActivePppoeUsernames(concentrator)` UMA vez por concentrador (paralelo). Quando um link offline é filtrado por ter PPPoE ativo, registra evento `ip_mismatch` (deduplicado via `getLatestUnresolvedLinkEvent`). Para Cisco, o detector continua dependendo de RADIUS accounting (não há equivalente bulk SNMP barato em concentradores Cisco grandes).
