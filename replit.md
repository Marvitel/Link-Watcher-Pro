...
Sincronização horária do status técnico de cada conexão Voalle (distinto do status comercial em `contractStatus`). Endpoint: `GET /external/map/connection/all` no Voalle (path raiz `:45715/external/map/...` — diferente do `apiRequest` padrão `:45715/external/integrations/thirdparty/...`, por isso o adapter tem método separado `mapApiRequest`).

**Schema** (`shared/schema.ts`): coluna `voalle_connection_status` em `links` com valores `normal` (1), `blocked` (2), `block_warning` (3), `maintenance_warning` (4), `unknown` (default), e `voalle_connection_status_updated_at`. Migration idempotente em `scripts/add-voalle-connection-status.sql` (aplicar em produção via `psql "$DATABASE_URL" -f ...`). Campo `voalleConnectionStatus` também exposto em `LinkDashboardItem` (interface compartilhada).

**Backend**:
- `VoalleAdapter.getAllConnectionStatus()` retorna array `{ id, status, statusRaw, user?, serviceTag? }` mapeando o numérico Voalle → string interna. **Propaga erro** em vez de silenciar — caller precisa diferenciar "0 conexões" (sucesso) de "falha de integração".
- `server/voalle-connection-sync.ts` orquestra: lê adapter ativo (`provider='voalle' & isActive=true`), chama o endpoint, faz match por `voalleConnectionId` e dispara `storage.bulkUpdateVoalleConnectionStatus()`. Marca `ok=false` em qualquer falha.
