...
**Auto-discovery de ifIndex (failover entre métodos)**: quando a coleta SNMP de um link PPPoE falha N vezes seguidas (`IFINDEX_MISMATCH_THRESHOLD`), o `handleIfIndexAutoDiscovery` (server/monitoring.ts) tenta recuperar a interface correta via, em ordem: lookup por nome de interface no concentrador → lookup PPPoE direto via SNMP/SSH → IP route lookup (`ipCidrRouteIfIndex` /32 + fallback `ipRouteIfIndex`) no concentrador primário → mesmo fluxo no concentrador backup. **Sanity check obrigatório no IP route lookup** (helper `isPlausibleSubscriberInterface`): para links PPPoE/com `pppoeUser`, a interface descoberta TEM que parecer interface de sessão (`Vi*`, `Virtual-Access*`, `Virtual-Template*`, `BVI*`, `Dialer*`, `pppoe-*`, `<pppoe-*`, `ppp\d`) OU conter o `pppoeUser` como substring. Se vier `sfpplus-vlan*`, `ether*`, `gigabit*`, `bridge*`, etc, a descoberta é REJEITADA — caso contrário a rota agregada do pool PPPoE (`/24` apontando para o uplink do BNG) trocaria a interface do assinante pela interface de uplink, fazendo o link medir o tráfego de centenas de clientes. Quando algum link já foi corrompido por essa via antes da proteção, usar `scripts/recover-ifindex-aggregate-route-bug.sql` para resetar `snmp_interface_index = NULL` (a auto-discovery roda de novo no próximo ciclo, agora pelo caminho correto).
...
- `server/voalle-connection-sync.ts` orquestra: lê adapter ativo (`provider='voalle' & isActive=true`), chama os dois endpoints **em paralelo** (`Promise.all`), faz match por `voalleConnectionId` e dispara `storage.bulkUpdateVoalleConnectionStatus()`. Marca `ok=false` se qualquer um dos endpoints falhar.
- **Regra de prioridade ativas↔excluídas**: o Voalle mantém histórico em `/all/deleted` mesmo depois de uma conexão ser reativada — então quando o mesmo `voalleConnectionId` aparece nas DUAS listas, vence a ATIVA (estado atual do Voalle). Só marca como `deleted` quem aparece EXCLUSIVAMENTE em `/all/deleted`. O log do sync inclui o contador de overlap ignorado para auditoria.
- **Diagnóstico**: `GET /api/admin/diagnostics/voalle-connection/:voalleId` (token de diagnóstico) retorna se o ID aparece em `/all`, `/all/deleted` ou ambos, junto com os links internos vinculados — útil para investigar divergências de status.
